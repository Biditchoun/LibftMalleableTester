...
- When an unexpected result is encountered, the program will show directly what the expected result was compared to the obtained one. This might not always work properly with e.g. empty strings.
...
- If a test provokes a bus error or a segfault, you will have to search for it by yourself by fiddling with the tester. The tester will often segfault when your function sends back NULL instead of something else, since it will try to use said NULL in strcmp.
